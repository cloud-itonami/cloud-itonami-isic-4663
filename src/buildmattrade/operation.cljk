(ns buildmattrade.operation
  "OperationActor -- one building-materials-wholesale operation = one
  supervised actor run, expressed as a langgraph-clj StateGraph. The
  advisor (BuildMatTradeAdvisor) is sealed into a single node
  (:advise); its proposal is ALWAYS routed through the Potable Water
  ...
  One graph run = one building-materials-wholesale operation (intake ->
  advise -> govern -> decide -> commit | hold | approval). No unbounded
  inner loop -- each operation is auditable and checkpointed. A
  building-order's life is advanced by MANY ops (intake / verify /
  ...
  decision to a human operator (trading supervisor / yard operator). The
  approver resumes with `{:approval {:status :approved}}` (or
  :rejected). `:delivery/dispatch`/`:invoice/settle` ALWAYS reach this
  node when the governor is clean -- see `buildmattrade.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [buildmattrade.buildmattradeadvisor :as buildmattradeadvisor]
...
(defn build
  ...
  `buildmattrade.store/Store`).
  opts:
    :advisor      -- a `buildmattrade.buildmattradeadvisor/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (buildmattradeadvisor/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; BuildMatTradeAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (buildmattradeadvisor/-advise advisor store request)]
            {:proposal p :audit [(buildmattradeadvisor/trace request p)]})))

      ;; Potable Water Safety Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
